...
const MembershipPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setSubmitting(false);
      setForm({ fullName: "", phone: "", email: "", budgetRange: "", purpose: "", locations: [], timeline: "", agreed: false });
    }, 1500);
  };
        ...
        <div className="section-container max-w-2xl">
          ...
          >
            <div>
              ...
              <input type="text" required value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} className={inputClass} placeholder="Your full name" />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-sm text-cream-muted mb-1.5 block">Phone Number</label>
                <input type="tel" required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={inputClass} placeholder="+91 XXXXX XXXXX" />
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {budgetRanges.map((b) => (
                  <button key={b} type="button" onClick={() => setForm({ ...form, budgetRange: b })}
                    className={`px-4 py-2 rounded-lg text-sm transition-all duration-200 ${form.budgetRange === b ? "gold-gradient-bg text-accent-foreground font-medium" : "glass-card text-cream-muted hover:border-gold/30"}`}
                  >{b}</button>
                ))}
              </div>
            </div>
              ...
              <div className="flex gap-2">
                {purposes.map((p) => (
                  <button key={p} type="button" onClick={() => setForm({ ...form, purpose: p })}
                    className={`px-4 py-2 rounded-lg text-sm transition-all duration-200 ${form.purpose === p ? "gold-gradient-bg text-accent-foreground font-medium" : "glass-card text-cream-muted hover:border-gold/30"}`}
                  >{p}</button>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {locations.map((loc) => (
                  <button key={loc} type="button" onClick={() => handleLocationToggle(loc)}
                    className={`px-4 py-2 rounded-lg text-sm transition-all duration-200 ${form.locations.includes(loc) ? "gold-gradient-bg text-accent-foreground font-medium" : "glass-card text-cream-muted hover:border-gold/30"}`}
                  >{loc}</button>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {timelines.map((t) => (
                  <button key={t} type="button" onClick={() => setForm({ ...form, timeline: t })}
                    className={`px-4 py-2 rounded-lg text-sm transition-all duration-200 ${form.timeline === t ? "gold-gradient-bg text-accent-foreground font-medium" : "glass-card text-cream-muted hover:border-gold/30"}`}
                  >{t}</button>
                ))}
              </div>
            </div>
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm text-cream-muted">
                I agree to the membership terms and acknowledge that Crestline Capital is a buyer aggregation platform, not a financial advisor or real estate broker.
              </span>
            </label>
